...
public  class OrgChartPrinter implements OrgChartPrinter__Annotations {
	...
	public StringBuilder print ( ) throws Exception_InvalidArgumentPassed {
		int numColumns = pass1_CalculateNumColumnsForNode ( OrgChartPrinter__MatrixWorker.accessMatrix_OrgChart__19(executeInfo, this), 1 ) ;
	...
	public int pass1_CalculateNumColumnsForNode ( net.unconventionalthinking.matrix.Descriptor currEmployee , int currTreeDepth ) throws Exception_InvalidArgumentPassed {
		...
		if ( currEmployee . get_ChildDescriptors_Size ( executeInfo ) == 0 ) {
			((com.mycompany.hr.FieldSetTuple__com$__$mycompany$__$hr$_CC_$OrgChart$__$Schema$_S_$EMPLOYEE)OrgChartPrinter__MatrixWorker.accessMatrixForFieldAssign___20(executeInfo, currEmployee, this).get_FieldSet()).set_NumColumns(executeInfo, 1 );
			return 1 ;
			
		}
		else {
			int sum_ChildNumCols = 0 ;
			for ( net.unconventionalthinking.matrix.Descriptor childEmployee : OrgChartPrinter__MatrixWorker.accessMatrix___21(executeInfo, currEmployee, this, AppSymbols_DescTagName.EMPLOYEE)) {
				sum_ChildNumCols  = sum_ChildNumCols + pass1_CalculateNumColumnsForNode ( childEmployee , currTreeDepth + 1 ) ;
				
			}
			((com.mycompany.hr.FieldSetTuple__com$__$mycompany$__$hr$_CC_$OrgChart$__$Schema$_S_$EMPLOYEE)OrgChartPrinter__MatrixWorker.accessMatrixForFieldAssign___22(executeInfo, currEmployee, this).get_FieldSet()).set_NumColumns(executeInfo, sum_ChildNumCols);
	...
	public void pass2_CreateBuildTableQueue ( ) throws Exception_InvalidArgumentPassed {
		EmployeeNode rootEmployeeNode = new EmployeeNode ( OrgChartPrinter__MatrixWorker.accessMatrix_OrgChart__23(executeInfo, this), 1 ) ;
	...
	public StringBuilder pass3_PrintTable ( ) {
		...
		currEmployeeNode  = buildTable_Queue . pollFirst ( ) ) {
			...
			int colSpan = currEmployeeDesc != null ? ( Integer ) OrgChartPrinter__MatrixWorker.accessMatrix___24(executeInfo, currEmployeeDesc, this): 1 ;
			if ( currEmployeeDesc != null ) {
				out . append ( "\t<td colspan='" + colSpan + "' style='text-align: center; border:solid;'>\n" ) ;
				out . append ( "\t\t<b>" + OrgChartPrinter__MatrixWorker.accessMatrix___25(executeInfo, currEmployeeDesc, this)+ "</b><br>\n" ) ;
				out . append ( "\t\t" + OrgChartPrinter__MatrixWorker.accessMatrix___26(executeInfo, currEmployeeDesc, this)+ " (<i>" + currEmployeeDesc . get_Label ( ) . toString ( ) + "</i>)\n" ) ;
