...
public class FieldSetTuple__com$__$mycompany$__$hr$_CC_$OrgChart$__$Schema$_S_$EMPLOYEE extends FieldSet_Tuple {
	...
	int NumColumns;
	FieldConstants NumColumns__Special;
	...
	public int get_NumColumns(ExecutionInfo executeInfo) throws Exception_InvalidArgumentPassed  {

		if (NumColumns__Special == FieldConstants.HAS_STANDARD_VALUE) {
			executeInfo.clearResultsInfo();
			return NumColumns;

		} else if (NumColumns__Special == FieldConstants.UNUSED) {
			executeInfo.setResultsInfoToOneValue(ExecutionInfo.ResultType.FieldValue_UNUSED);
			return (int)-1;

		} else if (NumColumns__Special == FieldConstants.NULL) {
			executeInfo.setResultsInfoToOneValue(ExecutionInfo.ResultType.FieldValue_NULL);
			return (int)-1;
	...
	public void set_NumColumns(ExecutionInfo executeInfo, int value) throws Exception_InvalidArgumentPassed  {
		//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
		if (this.NumColumns__Special == null) {
			++fieldValue_TotalCount;
			++fieldValue_NonContinuationCount;
		}
		this.NumColumns = value;
		this.NumColumns__Special = FieldConstants.HAS_STANDARD_VALUE;
	}
	public void set_NumColumns__Special(ExecutionInfo executeInfo, FieldConstants specialValue)
			throws Exception_InvalidArgumentPassed  {
		//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
		if (this.NumColumns__Special == null) {
			++fieldValue_TotalCount;
		...
		this.NumColumns__Special = specialValue;
		this.NumColumns = -1;
	}
	...
	public void setField(ExecutionInfo executeInfo, DescriptorTagName fieldName, Object value) throws Exception_InvalidArgumentPassed  {
		...
		switch ( ((Symbol)fieldName).ID ) {
			case com.mycompany.hr.AppSymbols_DescTagName.FullName$_ID_$:
				//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
				if (this.FullName__Special == null) {
					++fieldValue_TotalCount;
					++fieldValue_NonContinuationCount;
				...
				this.FullName__Special = FieldConstants.HAS_STANDARD_VALUE;
				return;
			case com.mycompany.hr.AppSymbols_DescTagName.Title$_ID_$:
				//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
				if (this.Title__Special == null) {
					++fieldValue_TotalCount;
					++fieldValue_NonContinuationCount;
				...
				this.Title__Special = FieldConstants.HAS_STANDARD_VALUE;
				return;
			case com.mycompany.hr.AppSymbols_DescTagName.NumColumns$_ID_$:
				//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
				if (this.NumColumns__Special == null) {
					++fieldValue_TotalCount;
					++fieldValue_NonContinuationCount;
				}
				this.NumColumns = ((Integer)value).intValue();
				this.NumColumns__Special = FieldConstants.HAS_STANDARD_VALUE;
				return;
		...
		switch ( ((Symbol)fieldName).ID ) {
			case com.mycompany.hr.AppSymbols_DescTagName.FullName$_ID_$:
 			...
 			case com.mycompany.hr.AppSymbols_DescTagName.NumColumns$_ID_$:
				if (NumColumns__Special == FieldConstants.HAS_STANDARD_VALUE) {
					executeInfo.clearResultsInfo();
					return new Integer((int)NumColumns);
		...
		if (index < schemaInfo_Positioned.getNum_NonContinuationFields()) {
			switch (index) {
				case 0:
 				...
 				case 2:
					//  if the previous fieldValue special has not been set, then you are adding a new value. increase count
					if (NumColumns__Special == null) {
						++fieldValue_TotalCount;
						++fieldValue_NonContinuationCount;
					}
					NumColumns = ((Integer)value).intValue();
					NumColumns__Special = FieldConstants.HAS_STANDARD_VALUE;
					return;
			...
			switch (index) {
				case 0:
 				...
 				case 2:
					...
					NumColumns__Special = specialValue;
					NumColumns = -1;
					return;
