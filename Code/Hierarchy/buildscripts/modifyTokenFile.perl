...
# Read the Token.java file, and add the statement, "import net.unconventionalthinking.compiler.tools..."
...
while (<TokenFileIn>) {
	...
	if ($_ =~ /import com\.unconventionalthinking\.compiler\.tools/) {
		$fileAlreadyModified = 1; #true 
	}
			
	if ($fileAlreadyModified == 0) {
		if ($_ =~/^\@SuppressWarnings/) {
			print TokenFileOut "import net.unconventionalthinking.compiler.tools.*;\n\n" .
