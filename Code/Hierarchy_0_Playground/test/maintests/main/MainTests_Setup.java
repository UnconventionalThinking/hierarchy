...
public class MainTests_Setup {
    ...
    public static HierarchyMetaCompiler setup_HierarchyCompilerForTests(boolean building_PersistentMatrix, boolean cleanUpOldFiles) throws Exception {
        ...
        hierarchyCompiler.set_SrcMetaCode_BaseDir(MainTests_Constants.maintests_DirPath);
        hierarchyCompiler.set_SrcMetaCode_BaseDir(MainTests_Constants.maintests_DirPath);
        hierarchyCompiler.set_InitiallyCompileAllJavaCode(false);
        List<String> selectedClassesToCompile = HierarchyMetaCompiler_Configurator.parseClassList_intoStrings(
            "maintests.samples.TestConstants");
        hierarchyCompiler.set_InitiallyCompiledJava_SelectedClasses_List(selectedClassesToCompile);

        hierarchyCompiler.set_AppClasses_PackageName(MainTests_Constants.maintests_generatedCode_PackageName);

        // NOTE: output the generated meta files back to the test dir, so they can be used to run the embedded code test
        hierarchyCompiler.set_BuildMeta_BaseDir(MainTests_Constants.maintests_DirPath);
        hierarchyCompiler.set_BuildFinal_BaseDir(MainTests_Constants.maintests_BuildFinal_DirPath);
